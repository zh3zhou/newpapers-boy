...
- 艺术来源：使用 Agent Web Search 主动拓展，每日至少覆盖 2 个独立来源/机构，同一来源原则上最多 1 条；在博物馆、美术馆、艺术节、摄影机构、艺术媒体、艺术家或项目官方页面之间轮换，避免连续多日只用 MoMA，不使用自建爬虫
...
> 所有路径默认相对于项目根目录。脚本会从自身位置推导默认根目录，也可统一用 `--root` 指向另一份项目副本。
...
- 目标送达时间：北京时间 07:00 左右
- GitHub Actions 触发时间：北京时间 06:30（UTC `30 22 * * *`），为检索、验证、TTS 和邮件预留处理时间；平台排队可能造成延迟
