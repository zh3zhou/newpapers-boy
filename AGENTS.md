...
- 交互语言不等于速递内容语言。速递正文仍以 `config.md` 第六节为准；未修改配置时保持中文正文、英文论文题目和专有名词。
- CI / 非交互模式不输出配置向导，只按契约生成文件和日志。

### 0.2 Agent-first 部署原则

本项目的首选部署方式，是让具备文件读取、终端执行和必要联网能力的 agent 读取本文件、`README.md` 与 `config.md` 后完成环境检查和引导配置。用户不需要先理解虚拟环境、SMTP、GitHub Actions 或 runner。
...
- 打岔内容：
  - 艺术一刻：2-3 条，可放宽到近期。必须使用 agent 的广域网页搜索，不得用自建爬虫或固定抓取单一站点；至少覆盖 2 个相互独立的来源/机构，同一来源原则上最多 1 条。优先轮换可信博物馆、美术馆、艺术节、摄影机构、艺术媒体及艺术家/项目官方页面，避免连续多日依赖 MoMA；若只有 1 条则宁缺毋滥。
...
- 定时触发：UTC `30 22 * * *`，对应北京时间 06:30；为 agent 检索、验证、TTS 和邮件预留约 30 分钟，目标在北京时间 07:00 左右送达。GitHub schedule 可能因平台排队延迟，不能承诺精确到分钟。
